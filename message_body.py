...
class RREQ(ISerializable):  # 파일 전송 요청 메세지(0x01)에 사용할 본문 클래스이다. FILESIZE와 FILENAME 데이터 속성을 갖는다.
    def __init__(self, buffer):
        if buffer != None:
            ...
            self.struct_fmt = str.format('=sssssi', len(buffer))
            self.struct_len = struct.calcsize(self.struct_fmt)

            unpacked = struct.unpack(self.struct_fmt, buffer)
            print('unpack', len(unpacked))
            self.initiator = unpacked[0]
            self.target = unpacked[1].decode(encoding='utf-8').replace('\x00', '')
            self.sender = unpacked[2]
            self.path = unpacked[1].decode(encoding='utf-8').replace('\x00', '')
            self.path_balance = unpacked[2]
            self.path_balance = unpacked[2]
        else:
